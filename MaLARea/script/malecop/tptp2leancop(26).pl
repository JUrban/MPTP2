%% File: tptp2leancop.pl  -  Version: 1.29  -  Date: 29 February 2012
%%
...
tptp2leancop_main(Files,Settings):-
	findall((File,R),(member(File,Files),tptp2leancop_database(File,Settings,R)),Rs),
...
%    (C1=[#|_] -> AX=conjecture ; AX=axiom),
    ...
    (ground(C) -> G=g ; G=n),
    assert_renvar(C1,C2), 
    (dnf(C2,C1_,G_,Index,Fs), =@@@=(C1_,C1) -> %writeln(Index),
        % It is already in database
        assert(dnf(C2,C1,G,Index,Features))   
      ; flag(index_of_dnf,Index,Index+1),
        assert(dnf(C2,C1,G,Index,Features))
     ),
     (nonvar(I) ->
         %retract(dnf(_,_,_,Index,_)),
